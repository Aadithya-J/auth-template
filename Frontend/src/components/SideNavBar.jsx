...
export default function SideNavBar({ onToggle, handleLogout }) {
  ...
  return (
    <aside 
      className={`h-screen ${expand ? "w-64" : "w-24"} bg-white  flex flex-col transition-all duration-500 ease-in-out fixed left-0 top-0 z-10  overflow-hidden`}
    > 
      {/* Logo and Toggle */}
      <div className={`flex items-center ${expand ? "justify-between" : "justify-between"} py-5 px-4 relative`}>
        <div className="flex items-center cursor-pointer group" onClick={() => navigate("/")}> 
          <img src={logo} alt="Logo" className="w-9 h-9 min-w-[36px]" />
          <div className={`transition-all duration-500 ease-in-out overflow-hidden ${expand ? "w-auto opacity-100 ml-3" : "w-0 opacity-0 ml-0"}`}>
      ...
      <div className="mt-2 px-3">
        <div className={`overflow-hidden transition-all duration-500 ease-in-out ${expand ? "max-h-6 opacity-100 mb-4" : "max-h-0 opacity-0"}`}>
      ...
      <div className="mt-auto px-3">
        <div className={`overflow-hidden transition-all duration-500 ease-in-out ${expand ? "max-h-6 opacity-100 mb-3 pt-4" : "max-h-0 opacity-0"}`}>
          <span className="text-xs font-medium text-blue-400 uppercase tracking-wider">
            Account
          </span>
        </div>
        ...
        <div className="border-t border-gray-100 pt-3 pb-2">
          <div className="flex items-center">
            <div className="relative">
              <img src={profile} alt="Profile" className="w-9 h-9 rounded-full object-cover border-2 border-blue-100" />
              <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border border-white"></div>
            </div>
            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${expand ? "max-w-[160px] opacity-100 ml-3" : "max-w-0 opacity-0"}`}>
              <h2 className="font-medium text-gray-800 text-sm line-clamp-1">{userDetails.name}</h2>
              <span className="text-gray-500 text-xs line-clamp-1">{userDetails.email}</span>
            </div>
          </div>
          ...
          >
            <FiLogOut size={18} />
            <span className={`transition-all duration-500 ease-in-out overflow-hidden ${expand ? "max-w-[80px] opacity-100 ml-3" : "max-w-0 opacity-0"}`}>
              Logout
            </span>
          </button>
        </div>
...
export function SideNavBarItem({ icon, text, route, expand, activeItem, onClick }) {
  ...
  return (
    <li
      className={`flex items-center ${expand ? "px-3" : "justify-center"} py-3 rounded-lg cursor-pointer transition-all duration-300 
      ${isActive 
        ? "bg-gradient-to-r from-blue-600 to-blue-500 text-white shadow-sm" 
        : "text-gray-600 hover:bg-blue-50"}`}
      onClick={() => onClick(route)}
    >
      <div className={`${isActive ? "text-white" : "text-blue-500"} transition-colors duration-300`}>
        {icon}
      </div>
      <div className={`transition-all duration-500 ease-in-out overflow-hidden ${expand ? "max-w-[160px] opacity-100 ml-3" : "max-w-0 opacity-0"}`}>
