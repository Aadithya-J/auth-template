...
import { useState, useEffect, useRef } from "react";
import axios from "axios";
import { Mic, MicOff, Check, X, Volume2, ArrowRight, SkipForward } from "lucide-react";
import { backendURL, pythonURL } from "../../definedURL";
import { motion, AnimatePresence } from "framer-motion";
import PropTypes from "prop-types";

// WORDS array and other components remain unchanged...
const WORDS = [
  {
    id: 1,
    sounds: ["/sounds/c.mp3", "/sounds/a.mp3", "/sounds/t.mp3"],
...
// ProgressBar, ResultCard, and Button components remain unchanged...
const ProgressBar = ({ progress }) => (
  <div className="w-full bg-blue-100 rounded-full h-2.5 overflow-hidden">
    <motion.div
      className="bg-blue-600 h-2.5"
...
const Button = ({
  ...
  const variants = {
    ...
    danger: "bg-red-600 text-white hover:bg-red-700 active:scale-98",
    success: "bg-green-600 text-white hover:bg-green-700 active:scale-98",
    warning: "bg-yellow-500 text-white hover:bg-yellow-600 active:scale-98",
  };
...
export default function PhonemeGame({
  ...
  const [readyForNext, setReadyForNext] = useState(false); // New state to track if ready for next question
  const [showFinalSubmit, setShowFinalSubmit] = useState(false);
  const audioRef = useRef(null);
  const mediaRecorderRef = useRef(null);
  ...
  const moveToNextWord = () => {
    ...
    const isCorrect = finalResponse === currentWord.word.toLowerCase();
  ...
  // New function to skip the current word
  const skipWord = () => {
    const currentWord = WORDS[currentWordIndex];
        ...
        {!showResponse ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col items-center space-y-3">
              ...
              {showFinalSubmit && (
                <Button onClick={handleFinalSubmit} variant="primary" className="w-full">
                  Submit All Results
                </Button>
              )}
            </div>
