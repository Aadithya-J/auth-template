    ...
    <div className="mb-6">
      ...
      >
        <textarea
          ...
          onChange={(e) => setCurrentDefinition(e.target.value)}
          placeholder="Enter the definition here or use the microphone..."
          className={`w-full px-3 py-2 text-blue-800 border ${isListening ? 'border-red-400' : 'border-blue-300'} rounded-lg focus:outline-none focus:border-blue-500 transition-colors duration-300`}
          disabled={isSubmitting || isListening || isProcessing}
        />
...
// Navigation Button Component
const NavigationButton = ({ onClick, isLast, isDisabled, isSubmitting, isListening, isProcessing, incorrectStreak }) => {
  const isFinish = isLast || incorrectStreak >= 4;
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      onClick={onClick}
      disabled={isDisabled || isSubmitting || isListening || isProcessing}
      className={`bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg focus:outline-none focus:shadow-outline transition-all duration-300 flex items-center gap-2 ${isDisabled || isSubmitting || isListening || isProcessing ? 'opacity-50 cursor-not-allowed' : 'shadow-md'}`}
    >
      {isSubmitting ? (
        <>
          <motion.div
...
const VocabularyScaleTest = () => {
  ...
  // Handle transcribed audio
  const handleTranscriptionComplete = useCallback((transcription) => {
    const cleanedTranscript = transcription
      .toLowerCase()
      .trim()
      .replace(/[.,!?;:]*$/, '');
    setCurrentDefinition(cleanedTranscript);
  }, []);
  ...
  // Combine component error with speech recognition error
  useEffect(() => {
    if (speechError) {
  ...
  if (error && !isLoading && !isSubmitting && !isProcessing) {
    return <ErrorState message={error} />;
  }
