...
const parseTranscript = (transcript) => {
  ...
  toast.warn(
    "Couldn't understand the numbers clearly. Please try speaking again."
  );
...
function Test13({ suppressResultPage = false, onComplete }) {
  ...
  const evaluateAnswer = useCallback(() => {
    ...
    if (userAnswer.length === 0) {
      toast.warning(
        "Couldn't understand the numbers clearly. Let's try that sequence again."
      );
      setTranscript("");
  ...
  useEffect(() => {
    if (gameState === "listening") {
      ...
      speakText(
        mode === "forward"
          ? "Your turn. Say the numbers."
          : "Your turn. Say the numbers backwards."
      );
  ...
  const handleStartForward = useCallback(() => {
    speakText(
      "Okay, let&apos;s start with forward numbers. Listen carefully, then say the numbers just like I do. Ready?"
    );
    startTest("forward");
  }, [startTest]);
  ...
  const handleStartReverse = useCallback(() => {
    speakText(
      "Great! Now for backwards numbers. When I say numbers, you say them the other way round. If I say 1, 3... you say 3, 1. Understand? Let&apos;s try."
    );
    startTest("reverse");
  }, [startTest]);
  ...
  const submitResults = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        "An error occurred while submitting. Please check connection or try again later."
      );
    }
  ...
  const renderInstructions = () => (
    ...
    >
      ...
      <h2 className="text-4xl font-bold text-gray-800 mb-8">Memory Test</h2>
        ...
        <ul className="grid grid-cols-1 gap-6 max-w-2xl mx-auto">
          <li className="flex items-center gap-4 bg-blue-50 p-6 rounded-xl">
            ...
            <span className="text-lg text-gray-700">
              Listen carefully to the numbers I say
            </span>
          </li>
          <li className="flex items-center gap-4 bg-blue-50 p-6 rounded-xl">
            <span className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full bg-blue-600 text-white font-semibold text-lg">
            ...
            <span className="text-lg text-gray-700">
              When I finish, repeat them back exactly
            </span>
          </li>
          <li className="flex items-center gap-4 bg-blue-50 p-6 rounded-xl">
            <span className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full bg-blue-600 text-white font-semibold text-lg">
  ...
  const renderListening = () => (
    ...
    >
      <h3 className="text-2xl font-bold text-gray-800">
        {mode === "forward"
          ? "Repeat the numbers in order"
          : "Say the numbers in reverse"}
      </h3>
      ...
      {transcript && !isTranscribing && (
        ...
        >
          <p className="text-lg text-gray-600">
            You said: <strong className="text-gray-800">{transcript}</strong>
          </p>
        </motion.div>
      )}
